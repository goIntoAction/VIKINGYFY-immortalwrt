...
configure_wifi() {
    ...
    uci set wireless.default_radio${radio}.key=${BASE_PASSWORD}
...
set_wifi_def_cfg() {
    ...
    '5g')
        ...
        if [ -z "$FIRST_5G" ]; then
            [ "$RADIO_NUM" -gt 2 ] && WIFI_SSID="${BASE_SSID}_5G1" || WIFI_SSID="${BASE_SSID}_5G"
            FIRST_5G=1
        else
            WIFI_SSID="${BASE_SSID}_5G2"
        fi
        ;;
    *)
        case "$htmode" in
